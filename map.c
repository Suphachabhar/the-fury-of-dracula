...
map *map_new(void)
{
	...
	(*g) = (map){
		...
		.n_edges = 0,
		.connections = {NULL},
		.vampire_location = UNKNOWN_LOCATION};
...
{
	return g->vampire_location;
}

void set_vampire_location(map *g, location_t new_location)
{
	g->vampire_location = new_location;
}

size_t get_traps(map *g, location_t location)
{
	if (valid_location_p(location))
		return g->traps[location];
	else
		return 0;
}
